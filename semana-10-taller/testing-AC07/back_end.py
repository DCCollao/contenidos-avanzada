...
class Programadito(QObject):
    ...
    def __init__(self):
        ...
        self.senal_mostrar_mensaje_error = None
    ...
    def ingresar_nombres(self, nombre_1, nombre_2):
        if nombre_1 == "" or nombre_2 == "":
            self.emit(self.senal_mostrar_mensaje_error, 1)
        elif not nombre_1.isalnum() or not nombre_2.isalnum():
            self.emit(self.senal_mostrar_mensaje_error, 2)
        elif nombre_1 == nombre_2:
            self.emit(self.senal_mostrar_mensaje_error, 3)
        else:
    ...
    def tirar_carta(self, n_jugador):
        ...
        if len(mazo) == 0:
            self.emit(self.senal_ganador, jugador, 1)
            self.juego_comenzo = False
    ...
    def juntar_cartas(self, jugador):
        if not self.juego_comenzo:
            return
        if self.carta_tirada and self.carta_contada in self.carta_tirada:
            if jugador == 1:
                self.emit(self.senal_ganador, self.nombre_jugador_1, 2)
                self.mazo_jugador_2.extend(self.cartas_pozo)
                self.turno_primero = False
            else:
                self.emit(self.senal_ganador, self.nombre_jugador_2, 1)
                self.mazo_jugador_1.extend(self.cartas_pozo)
                self.turno_primero = True
        else:  # Jugador presiono boton cuando no correspondia
            if jugador == 1:
                self.emit(self.senal_ganador, self.nombre_jugador_1, 3)
                self.mazo_jugador_1.extend(self.cartas_pozo)
                self.turno_primero = True
            else:
                self.emit(self.senal_ganador, self.nombre_jugador_2, 3)
                self.mazo_jugador_2.extend(self.cartas_pozo)
                self.turno_primero = False
        self.cartas_pozo = []
        self.carta_tirada = None
    ...
    def actualizar_interfaz(self):
        ...
        self.emit(self.senal_actualizar, estado)

    def emit(self, senal, *args):
        if senal:
            senal.emit(*args)
